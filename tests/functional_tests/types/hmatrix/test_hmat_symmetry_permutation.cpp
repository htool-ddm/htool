...
int main(int argc, char *argv[]) {
    ...
    for (int idist = 0; idist < ndistance; idist++) {
        ...
        GeneratorTestDouble A(3, nr, nr, p1, p1, 0.1);
        ...
        MPI_Bcast(f.data(), nr * mu, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        // Global with permutation
        std::vector<double> f_vec(f.begin(), f.begin() + nr);
        std::vector<double>
        ...
        std::vector<double> f_vec_local(f.begin() + MasterOffset_target[2 * rank], f.begin() + MasterOffset_target[2 * rank] + MasterOffset_target[2 * rank + 1]);
        std::vector<double> f_local(MasterOffset_target[2 * rank + 1] * mu);
        for (int i = 0; i < mu; i++) {
            std::copy_n(f.data() + MasterOffset_target[2 * rank] + nr * i, MasterOffset_target[2 * rank + 1], f_local.data() + MasterOffset_target[2 * rank + 1] * i);
        }
        ...
        std::vector<double> f_vec_perm(nr, 0), f_perm(nr * mu, 0);
        HA.source_to_cluster_permutation(f.data(), f_vec_perm.data());
        for (int i = 0; i < mu; i++) {
            HA.source_to_cluster_permutation(f.data() + nr * i, f_perm.data() + nr * i);
        }
        std::vector<double>
        ...
        for (int i = 0; i < mu; i++) {
            HA.local_source_to_local_cluster(f_local.data() + MasterOffset_target[2 * rank + 1] * i, f_local_perm.data() + MasterOffset_target[2 * rank + 1] * i);
        }
