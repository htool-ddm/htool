...
int test_solver_ddm(int argc, char *argv[], int mu, char symmetric, bool off_diagonal_approximation) {
    ...
    std::shared_ptr<Cluster<double>> target_cluster = std::make_shared<Cluster<double>>(
    ...
    DistributedOperator<Cplx> Operator(target_cluster, target_cluster);
    Operator.build_default_hierarchical_approximation(Generator, epsilon, eta);
    ...
    // Solve
    DDM<complex<double>> ddm_wo_overlap(&Operator);
    DDM<complex<double>> ddm_with_overlap(Generator, &Operator, ovr_subdomain_to_global, cluster_to_ovr_subdomain, neighbors, intersections);
