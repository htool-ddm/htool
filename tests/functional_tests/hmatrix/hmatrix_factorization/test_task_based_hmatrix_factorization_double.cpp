...
int main(int, char *[]) {
    ...
    for (auto epsilon : {1e-3, 1e-6}) {
        ...
        for (auto UPLO : {'L', 'U'}) {
            is_error = is_error || test_hmatrix_cholesky<htool::omp_task_policy<double> &&, double, GeneratorTestDoubleSymmetric>(omp_task_policy<double>{}, UPLO, n1, n2, epsilon, margin);
        }
    }
