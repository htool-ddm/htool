...
bool test_hmatrix_task_based(const TestCaseType &test_case, char trans_sym, char transa, htool::underlying_type<T> epsilon, bool block_tree_consistency, char UPLO = 'L') {
    bool is_error = false;
    double eta    = 10;
    ...
    if (trans_sym == 'N') {
        hmatrix_tree_builder = std::make_unique<HMatrixTreeBuilder<T, htool::underlying_type<T>>>(*root_cluster_A_output, *root_cluster_A_input, epsilon, eta, 'N', 'N', -1, -1, -1);
    } else if (trans_sym == 'S') {
        hmatrix_tree_builder = std::make_unique<HMatrixTreeBuilder<T, htool::underlying_type<T>>>(*root_cluster_A_output, *root_cluster_A_input, epsilon, eta, 'S', UPLO, -1, -1, -1);
    }
...
#    pragma omp single
        ...
        // save_leaves_with_rank(task_based_hmatrix, "TB_hmatrix_facto");
        // save_leaves_with_rank(hmatrix, "hmatrix_facto");
    ...
    // Tests for task_based_internal_add_hmatrix_vector_product
    {
        ...
        auto root_hmatrix_task_based = hmatrix_tree_builder->build(*test_case.operator_A, true); // not in parallel area because it is not the focus of the test, but should nevertheless be called with the flag true in order to build m_L0 needed later when : hmatrix_tree_builder->get_L0();
...
#    pragma omp single
            ...
            is_error = is_error || (norm2(out_task_con - out_task_dis) / norm2(out_task_con) > 1e-15);
        ...
        if (disjoint_case_duration.count() < conjoint_case_duration.count()) {
            std::cerr << "Careful: disjoint_case_TB_duration: " << disjoint_case_duration.count() << " < conjoint_case_TB_duration: " << conjoint_case_duration.count() << "." << std::endl;
        }
