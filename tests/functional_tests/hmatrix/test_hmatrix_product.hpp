...
bool test_hmatrix_product(int nr, int nc, int mu, bool use_local_cluster, char op, char Symmetry, char UPLO, htool::underlying_type<T> epsilon) {
    ...
    std::shared_ptr<Cluster<htool::underlying_type<T>>> target_root_cluster = make_shared<Cluster<htool::underlying_type<T>>>(recursive_build_strategy.create_cluster_tree(nr, 3, p1.data(), 2, sizeWorld, partition.data()));
    ...
    if (Symmetry == 'N' && nr != nc) {
        ...
        source_root_cluster = make_shared<Cluster<htool::underlying_type<T>>>(recursive_build_strategy.create_cluster_tree(nc, 3, p2.data(), 2, sizeWorld, partition.data()));
    } else {
    ...
    if (use_local_cluster) {
        hmatrix_tree_builder = std::make_unique<HMatrixTreeBuilder<T, htool::underlying_type<T>>>(target_root_cluster->get_cluster_on_partition(rankWorld), source_root_cluster->get_cluster_on_partition(rankWorld), epsilon, eta, Symmetry, UPLO, -1, -1);
    } else {
