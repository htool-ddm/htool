...
bool test_hmatrix_lrmat_product(const TestCaseProduct<T, GeneratorTestType> &test_case, bool use_local_cluster, htool::underlying_type<T> epsilon, htool::underlying_type<T> additional_lrmat_sum_tolerance) {
    ...
    SVD<T> compressor_B(*test_case.operator_B);
    LowRankMatrix<T> B_auto_approximation(root_cluster_B_output->get_size(), root_cluster_B_input->get_size(), lrmat_tolerance);
    compressor_B.copy_low_rank_approximation(root_cluster_B_output->get_size(), root_cluster_B_input->get_size(), root_cluster_B_output->get_offset(), root_cluster_B_input->get_offset(), B_auto_approximation);

    if (B_auto_approximation.rank_of() == 0) {
        int reqrank_B = (root_cluster_B_output->get_size() * root_cluster_B_input->get_size()) / (root_cluster_B_output->get_size() + root_cluster_B_input->get_size());
        compressor_B.copy_low_rank_approximation(root_cluster_B_output->get_size(), root_cluster_B_input->get_size(), root_cluster_B_output->get_offset(), root_cluster_B_input->get_offset(), reqrank_B, B_auto_approximation);
    }
    ...
    add_matrix_lrmat_product(transa, transb, alpha, A_dense, B_auto_approximation, beta, matrix_result_w_matrix_sum);
    add_matrix_lrmat_product(transa, transb, alpha, A_dense, B_auto_approximation, beta, matrix_result_w_lrmat_sum);
    add_matrix_lrmat_product(transa, transb, alpha, A_dense, B_auto_approximation, T(0), matrix_result_wo_sum);

    // Product
    matrix_test = C_dense;
    internal_add_hmatrix_lrmat_product(transa, transb, alpha, root_hmatrix, B_auto_approximation, beta, matrix_test);
    error    = normFrob(matrix_result_w_matrix_sum - matrix_test) / normFrob(matrix_result_w_matrix_sum);
    is_error = is_error || !(error < epsilon);
    cout << "> Errors on a hmatrix lrmat product to matrix: " << error << '\n';

    lrmat_test = C_auto_approximation;
    internal_add_hmatrix_lrmat_product(transa, transb, alpha, root_hmatrix, B_auto_approximation, T(0), lrmat_test);
    ...
    is_error = is_error || !(error < epsilon);
    cout << "> Errors on a hmatrix lrmat product to lrmat without lrmat sum: " << error << '\n';

    lrmat_test = C_auto_approximation;
    internal_add_hmatrix_lrmat_product(transa, transb, alpha, root_hmatrix, B_auto_approximation, beta, lrmat_test);
    dense_lrmat_test.resize(lrmat_test.get_U().nb_rows(), lrmat_test.get_V().nb_cols());
    lrmat_test.copy_to_dense(dense_lrmat_test.data());
    ...
    cout << "> Errors on a hmatrix lrmat product to lrmat with lrmat sum: " << error << '\n';

    hmatrix_test = C;
    internal_add_hmatrix_lrmat_product(transa, transb, alpha, root_hmatrix, B_auto_approximation, T(0), hmatrix_test);
    ...
    is_error = is_error || !(error < epsilon);
    cout << "> Errors on a hmatrix lrmat product to hmatrix without lrmat sum: " << error << '\n';

    hmatrix_test = C;
    save_leaves_with_rank(hmatrix_test, "hmatrix_test");
    internal_add_hmatrix_lrmat_product(transa, transb, alpha, root_hmatrix, B_auto_approximation, beta, hmatrix_test);
    copy_to_dense(hmatrix_test, densified_hmatrix_test.data());
    error    = normFrob(matrix_result_w_matrix_sum - densified_hmatrix_test) / normFrob(matrix_result_w_matrix_sum);
    is_error = is_error || !(error < std::max(epsilon, lrmat_tolerance) * (1 + additional_lrmat_sum_tolerance));
    cout << "> Errors on a hmatrix lrmat product to hmatrix with lrmat sum: " << error << '\n';
