            ...
            for (auto side : {'L', 'R'}) {
                for (auto operation : {'N', 'T', 'C'}) {
                    for (auto diag : {'N', 'U'}) {
                        ...
                        TestCaseSolve<std::complex<double>, GeneratorTestComplexHermitian> test_case(side, operation, n1, number_of_rhs, 1, -1);

                        is_error = is_error || test_task_based_hmatrix_triangular_solve<std::complex<double>, GeneratorTestComplexHermitian, TestCaseSolve<std::complex<double>, GeneratorTestComplexHermitian>>(test_case, side, operation, diag, epsilon);
                    }
                }
            }
        }
    ...
    if (false) {
        ...
        for (auto epsilon : {1e-3, 1e-8}) {
            for (auto trans : {'N', 'T'}) {
                ...
                // is_error = is_error || test_task_based_lu_factorization<std::complex<double>, GeneratorTestComplexHermitian>(trans, n1, n2, epsilon);
            }

            for (auto UPLO : {'L', 'U'}) {
                std::cout << "task based Cholesky factorization test case: " << "epsilon = " << epsilon << ", n1 = " << n1 << ", n2 = " << n2 << ", UPLO = " << UPLO << "\n";

                is_error = is_error || test_task_based_cholesky_factorization<std::complex<double>, GeneratorTestComplexHermitian>(UPLO, n1, n2, epsilon);
            }
        }
