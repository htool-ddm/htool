...
bool test_distributed_operator(int nr, int nc, int mu, bool use_permutation, char Symmetry, char UPLO, char op, bool off_diagonal_approximation, DataType data_type, htool::underlying_type<T> epsilon = 1e-14) {
    ...
    recursive_build.set_partition(sizeWorld, MasterOffset_target.data());
    std::shared_ptr<const Cluster<htool::underlying_type<T>>> source_root_cluster;
    std::shared_ptr<const Cluster<htool::underlying_type<T>>> target_root_cluster = make_shared<const Cluster<htool::underlying_type<T>>>(recursive_build.create_cluster_tree(nr, 3, p1.data(), 2, sizeWorld));
    ...
    if (Symmetry == 'N') {
        ...
        source_root_cluster = make_shared<const Cluster<htool::underlying_type<T>>>(recursive_build.create_cluster_tree(nc, 3, p2.data(), 2, sizeWorld));
    ...
    std::shared_ptr<PartitionFromCluster<T, htool::underlying_type<T>>> target_partition = std::make_shared<PartitionFromCluster<T, htool::underlying_type<T>>>(target_root_cluster);
    std::shared_ptr<PartitionFromCluster<T, htool::underlying_type<T>>> source_partition = std::make_shared<PartitionFromCluster<T, htool::underlying_type<T>>>(source_root_cluster);
    std::unique_ptr<DistributedOperator<T>> distributed_operator;

    if (data_type == DataType::DefaultHMatrix) {
        if (!off_diagonal_approximation) {
            distributed_operator = std::make_unique<DistributedOperator<T>>(build_default_hierarchical_approximation(generator_permuted, target_root_cluster, source_root_cluster, epsilon, eta, Symmetry, UPLO, MPI_COMM_WORLD));
        } else {
            distributed_operator = std::make_unique<DistributedOperator<T>>(build_default_local_hierarchical_approximation(generator_permuted, target_root_cluster, source_root_cluster, epsilon, eta, Symmetry, UPLO, MPI_COMM_WORLD));
        }
    } else {
        distributed_operator = std::make_unique<DistributedOperator<T>>(target_partition, source_partition, Symmetry, UPLO, MPI_COMM_WORLD);
    ...
    if (off_diagonal_approximation) {
        ...
        std::shared_ptr<const Cluster<htool::underlying_type<T>>> off_diagonal_cluster = make_shared<const Cluster<htool::underlying_type<T>>>(recursive_build.create_cluster_tree(nc, 3, p2_permuted.data(), 2, 3));
