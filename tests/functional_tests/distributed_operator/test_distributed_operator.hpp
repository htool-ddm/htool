...
#include <htool/distributed_operator/implementations/local_operators/local_dense_matrix.hpp> // for LocalDe...
#include <htool/distributed_operator/implementations/local_operators/local_hmatrix.hpp>      // for LocalHM...
#include <htool/distributed_operator/implementations/local_operators/local_operator.hpp>     // for LocalOp...
...
auto add_off_diagonal_operator(ClusterTreeBuilder<htool::underlying_type<T>> &recursive_build, DistributedOperator<T> &distributed_operator, const Cluster<htool::underlying_type<T>> &target_root_cluster, const std::vector<double> p1, const Cluster<htool::underlying_type<T>> &source_root_cluster, const std::vector<double> p2_permuted, DataType data_type, htool::underlying_type<T> epsilon, htool::underlying_type<T> eta) {
    struct Holder {
        ...
        Holder(ClusterTreeBuilder<htool::underlying_type<T>> &recursive_build, const Cluster<htool::underlying_type<T>> &target_root_cluster, const std::vector<double> &p1, const Cluster<htool::underlying_type<T>> &source_root_cluster, const std::vector<double> &p2_permuted, DataType data_type, htool::underlying_type<T> epsilon, htool::underlying_type<T> eta) {
            ...
            if (data_type == DataType::Matrix) {
                ...
                generator_off_diagonal->copy_submatrix(local_target_root_cluster.get_size(), local_off_diagonal_cluster_tree_1->get_size(), local_target_root_cluster.get_permutation().data() + local_target_root_cluster.get_offset(), local_off_diagonal_cluster_tree_1->get_permutation().data() + local_off_diagonal_cluster_tree_1->get_offset(), off_diagonal_matrix_1->data());

                generator_off_diagonal->copy_submatrix(local_target_root_cluster.get_size(), local_off_diagonal_cluster_tree_2->get_size(), local_target_root_cluster.get_permutation().data() + local_target_root_cluster.get_offset(), local_off_diagonal_cluster_tree_2->get_permutation().data() + local_off_diagonal_cluster_tree_2->get_offset(), off_diagonal_matrix_2->data());
    ...
    Holder holder(recursive_build, target_root_cluster, p1, source_root_cluster, p2_permuted, data_type, epsilon, eta);
...
bool test_default_distributed_operator(int nr, int nc, int mu, bool use_permutation, char Symmetry, char UPLO, char op, bool off_diagonal_approximation, DataType data_type, htool::underlying_type<T> epsilon = 1e-14) {
    ...
    if (off_diagonal_approximation) {
        ...
        auto dependencies                            = add_off_diagonal_operator<T, GeneratorTestType>(recursive_build, distributed_operator, *target_root_cluster, p1, *source_root_cluster, p2_permuted, data_type, epsilon, eta);
