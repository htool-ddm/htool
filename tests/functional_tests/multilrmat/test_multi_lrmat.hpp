...
int test_multi_lrmat(const MyMultiMatrix& A,const MultiLowRankMatrix<LowRankMatrix,T>& Fixed_approximation, const MultiLowRankMatrix<LowRankMatrix,T>& Auto_approximation, std::vector<int>& permt, std::vector<int>& perms, std::pair<double,double> fixed_compression_interval, std::pair<double,double> auto_compression_interval, int l){
	...
	double error=norm2(A.mult(f,l)-out_perm);
	...
	error = norm2(A.mult(f,l)-out_perm);
