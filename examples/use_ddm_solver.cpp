...
int main(int argc, char *argv[]) {
    ...
    DDMSolverBuilder<double> ddm_solver_build(distributed_operator, local_hmatrix);
    auto &solver = ddm_solver_build.solver;

    // Prepare solver
    HPDDM::Option &opt = *HPDDM::Option::get();
    opt.parse("-hpddm_krylov_method cg -hpddm_schwarz_method asm");
    if (rankWorld == 0) {
        opt.parse("-hpddm_verbosity 10");
    }
    if (rankWorld == 0) {
        std::cout << "Information about local hmatrix on rank 0 before factorization\n";
        print_hmatrix_information(local_hmatrix, std::cout);
    }
    solver.facto_one_level(); // block_diagonal_hmatrix is factorized in-place
    if (rankWorld == 0) {
        std::cout << "Information about local hmatrix on rank 0 after factorization\n";
        print_hmatrix_information(local_hmatrix, std::cout);
    }

    // Solve z = A^-1 y with PCG and block Jacobi
    int mu = 1; // number of right-hand side
    std::vector<double> z(number_points, 0);
    std::vector<double> b = y;
    solver.solve(b.data(), z.data(), mu);
    if (rankWorld == 0) {
        std::cout << "relative error on ddm solver : ";
