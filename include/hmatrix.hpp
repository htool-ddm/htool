...
std::pair<double,double> MvProdMPI(vectCplx& f, const HMatrix& A, const vectCplx& x, const MPI_Comm& comm){
	int rankWorld, sizeWorld;
   	MPI_Comm_size(comm, &sizeWorld);
   	MPI_Comm_rank(comm, &rankWorld);
...
Real CompressionRate(const HMatrix& hmat, const MPI_Comm& comm){
...
void print_stats(const HMatrix& A, const MPI_Comm& comm){
	int rankWorld;
    MPI_Comm_rank(comm, &rankWorld);
