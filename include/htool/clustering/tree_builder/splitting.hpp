...
class RegularSplitting {
  public:
    static void splitting(Cluster<T> *curr_cluster, const std::vector<int> &permutation, int spatial_dimension, const T *x, int current_number_of_children, int minclustersize, const std::vector<T> &dir, std::vector<std::pair<int, int>> &current_partition) {
    ...
    static void splitting(Cluster<T> *curr_cluster, const std::vector<int> &permutation, int spatial_dimension, const T *x, int current_number_of_children, int minclustersize, const std::vector<T> &dir, std::vector<std::pair<int, int>> &current_partition) {
        ...
        if (size > current_number_of_children) { // otherwise it won't be possible anyway
            ...
            if (std::all_of(sizes.begin(), sizes.end(), [&](int a) { return a > minclustersize; })) {
                ...
                RegularSplitting<T>::splitting(curr_cluster, permutation, spatial_dimension, x, current_number_of_children, minclustersize, dir, current_partition);
            }
        } else {
            current_partition.clear();
        }
