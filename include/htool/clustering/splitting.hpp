...
std::vector<std::vector<int>> geometric_splitting(int nb_pt0, const double *const x, const int *const tab, std::vector<int> &num, Cluster<ClusterImpl> const *const curr_cluster, int nb_sons, const std::vector<double> &dir) {
    ...
    // Otherwise we have to something more
        ...
        if (empty) {
            numbering = regular_splitting(nb_pt0, x, tab, num, curr_cluster, nb_sons, dir);
        }
    }
...
template <typename ClusterImpl>
std::vector<std::vector<int>> regular_splitting(int nb_pt0, const double *const x, const int *const tab, std::vector<int> &num, Cluster<ClusterImpl> const *const curr_cluster, int nb_sons, const std::vector<double> &dir) {
