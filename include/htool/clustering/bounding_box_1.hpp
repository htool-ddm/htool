...
class BoundingBox1 {
  public:
    void
    recursive_build(const double *const x, const double *const r, const int *const tab, const double *const g, int nb_sons, MPI_Comm comm, std::stack<Cluster<BoundingBox1> *> &s, std::stack<std::vector<int>> &n) {
        ...
        while (!s.empty()) {
            ...
            for (int j = 0; j < nb_pt; j++) {
                ...
                for (int p = 0; p < curr->get_space_dim(); p++) {
                    if (min_point[p] > x[curr->get_space_dim() * tab[num[j]] + p]) {
                        min_point[p] = x[curr->get_space_dim() * tab[num[j]] + p];
                    }
                    if (max_point[p] < x[curr->get_space_dim() * tab[num[j]] + p]) {
                        max_point[p] = x[curr->get_space_dim() * tab[num[j]] + p];
                    }
...
template <>
inline std::vector<std::vector<int>> BoundingBox1<SplittingTypes::GeometricSplitting>::splitting(const double *const x, const int *const tab, std::vector<int> &num, VirtualCluster const *const curr_cluster, int nb_sons, const std::vector<double> &dir) { return geometric_splitting(x, tab, num, curr_cluster, nb_sons, dir); }

template <>
inline std::vector<std::vector<int>> BoundingBox1<SplittingTypes::RegularSplitting>::splitting(const double *const x, const int *const tab, std::vector<int> &num, VirtualCluster const *const curr_cluster, int nb_sons, const std::vector<double> &dir) { return regular_splitting(x, tab, num, curr_cluster, nb_sons, dir); }
