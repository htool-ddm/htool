...
//=================================//
template <typename T>
class partialACA final : public LowRankMatrix<T> {
  ...
  public:
    ...
    void build(const VirtualGenerator<T> &A, const VirtualCluster &t, const double *const xt, const int *const tabt, const VirtualCluster &, const double *const, const int *const) {
        if (this->rank == 0) {
