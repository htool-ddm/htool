...
void test_partition(int spatial_dimension, int number_of_points, std::vector<CoordinatePrecision> &coordinates, int partition_size, std::vector<std::pair<int, int>> &partition) {
    ...
    for (int j = 0; j < number_of_points; j++) {
        std::vector<double> u(spatial_dimension, 0);
    ...
    std::sort(permutation.begin(), permutation.end(), [&](int a, int b) {
        double c = std::inner_product(coordinates.data() + spatial_dimension * a, coordinates.data() + spatial_dimension * (1 + a), direction.data(), double(0));
        double d = std::inner_product(coordinates.data() + spatial_dimension * b, coordinates.data() + spatial_dimension * (1 + b), direction.data(), double(0));
        return c < d;
    });
