  ...
  public:
    ...
    void add_vector_product_global_to_local(T alpha, int size_in, const T *const in, T beta, int size_out, T *const out) const override {
        ...
        if (m_target_use_permutation_to_mvprod) {
            global_to_cluster(m_cluster_tree_target.get(), out, buffer_out.data());
        }
    ...
    void add_matrix_product_global_to_local(T alpha, int size_in, const T *const in, T beta, int size_out, T *const out, int mu) const override {
        //
        int nc = m_cluster_tree_source->get_size();
        int nr = m_cluster_tree_target->get_size();
        ...
        if (m_source_use_permutation_to_mvprod) {
            for (int i = 0; i < mu; i++) {
                for (int j = 0; j < nc; j++) {
                    buffer_in[j] = in[i + (m_cluster_tree_source->get_offset() + j) * mu];
                }
                global_to_cluster(m_cluster_tree_source.get(), buffer_in.data(), buffer_in.data() + nc + nc * i);
            }
            for (int i = 0; i < mu; i++) {
                for (int j = 0; j < nc; j++) {
            ...
            std::copy_n(in + m_cluster_tree_source->get_offset() * mu, in_perm.size(), in_perm.data());
            conj_if_complex<T>(in_perm.data(), in_perm.size());
        }

        const T *input = m_source_use_permutation_to_mvprod || m_symmetry == 'H' ? in_perm.data() : in + m_cluster_tree_source->get_offset() * mu;

        // Local to local product
        if (m_symmetry == 'N') {
            m_data.add_matrix_product_row_major(alpha, input, beta, output, mu, 'T', 'N');
        } else if (m_symmetry == 'S' || m_symmetry == 'H') {
            m_data.add_matrix_product_symmetric_row_major(alpha, input, beta, output, mu, m_UPLO, m_symmetry);
        } else {
            throw std::invalid_argument("[Htool error] Invalid arguments for LocalDenseMatrix");
        }
        ...
        // if (m_target_use_permutation_to_mvprod) {
        //     for (int i = 0; i < mu; i++) {
        //         cluster_to_global(m_cluster_tree_target.get(), output + i * nr, out + i * nr);
        //     }
        // }
    }
    ...
    virtual void add_vector_product_transp_local_to_global(T alpha, int size_in, const T *const in, T beta, int size_out, T *const out) const override {
        ...
        if (m_source_use_permutation_to_mvprod) {
            cluster_to_global(m_cluster_tree_source.get(), out + m_cluster_tree_source->get_offset(), buffer_out.data());
        }
    ...
    virtual void add_matrix_product_transp_local_to_global(T alpha, int size_in, const T *const in, T beta, int size_out, T *const out, int mu) const override {
        ...
        // Local to local product
        if (m_symmetry == 'N') {
            m_data.add_matrix_product_row_major(alpha, input, beta, output_perm.data(), mu, 'T', 'T');
        } else if (m_symmetry == 'S' || m_symmetry == 'H') {
            m_data.add_matrix_product_symmetric_row_major(alpha, input, beta, output_perm.data(), mu, m_UPLO, m_symmetry);
        } else {
            throw std::invalid_argument("[Htool error] Invalid arguments for LocalDenseMatrix");
        }
        ...
        if (m_source_use_permutation_to_mvprod) {
            for (int i = 0; i < mu; i++) {
                ...
                global_to_cluster(m_cluster_tree_source.get(), buffer_out.data(), buffer_out.data() + nc + nc * i);
            }
            for (int i = 0; i < mu; i++) {
                for (int j = 0; j < nc; j++) {
                    out[i + (m_cluster_tree_source->get_offset() + j) * mu] = buffer_out[nc + j + i * nc];
                }
            }
        } else { // we have to conjugate to apply in this particular case
            std::copy_n(output_perm.data(), output_perm.size(), out + m_cluster_tree_source->get_offset() * mu);
        }
        if (m_symmetry == 'H') {
            conj_if_complex<T>(out + m_cluster_tree_source->get_offset() * mu, nc * mu);
        }
    }
