...
HMatrix<LowRankMatrix, T>::HMatrix(IMatrix<T> &mat,
                                   ...
                                   const std::vector<double> &gt,
                                   const int &reqrank0,
                                   MPI_Comm comm0) : nr(mat.nb_rows()), nc(mat.nb_cols()), cluster_tree_s(nullptr), cluster_tree_t(nullptr), reqrank(reqrank0) {
...
HMatrix<LowRankMatrix, T>::HMatrix(IMatrix<T> &mat,
                                   ...
                                   const std::vector<int> &tabt,
                                   const int &reqrank0,
                                   MPI_Comm comm0) : nr(mat.nb_rows()), nc(mat.nb_cols()), cluster_tree_s(nullptr), cluster_tree_t(nullptr), reqrank(reqrank0), comm(comm0) {
    ...
    if (B->IsAdmissible() && t.get_depth() >= GetMinTargetDepth() && s.get_depth() >= GetMinSourceDepth()) {
        ...
        if (t.IsLeaf()) {
            ...
            if ((bsize <= maxblocksize) && (r1 != NULL) && (r2 != NULL) && t.get_depth() >= GetMinTargetDepth() && s.get_depth() >= GetMinSourceDepth()) {
                ...
                if (r1 != NULL)
                    MyBlocks.push_back(r1);
                if (r2 != NULL)
                    MyBlocks.push_back(r2);
                delete B;
                return NULL;
            }
        }
        ...
        if (t.IsLeaf()) {
            ...
            if ((bsize <= maxblocksize) && (r3 != NULL) && (r4 != NULL) && t.get_depth() >= GetMinTargetDepth() && s.get_depth() >= GetMinSourceDepth()) {
                ...
                if (r3 != NULL)
                    MyBlocks.push_back(r3);
                if (r4 != NULL)
                    MyBlocks.push_back(r4);
                delete B;
                return NULL;
            }
        } else {
