...
template <typename T>
void add_symmetric_matrix_vector_product(char UPLO, T alpha, const Matrix<T> &A, const T *in, T beta, T *out) {
    int nr  = A.nb_rows();
    int lda = nr;
...
template <typename T>
void add_hermitian_matrix_vector_product(char UPLO, T alpha, const Matrix<T> &A, const T *in, T beta, T *out) {
    add_symmetric_matrix_vector_product(UPLO, alpha, A, in, beta, out);
}

template <typename T>
void add_hermitian_matrix_vector_product(char UPLO, std::complex<T> alpha, const Matrix<std::complex<T>> &A, const std::complex<T> *in, std::complex<T> beta, std::complex<T> *out) {
    int nr  = A.nb_rows();
    int lda = nr;
