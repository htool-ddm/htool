...
class Block {
  ...
  public:
    ...
    DenseBlockData<T> *get_dense_block_data() { return dynamic_cast<DenseBlockData<T> *>(block_data.get()); }
    LowRankMatrix<T> *get_low_rank_block_data() {
        return dynamic_cast<LowRankMatrix<T> *>(block_data.get());
        ;
    }
    ...
    // Sum Expressions
    // void set_sum_expressions(T alpha, block &A, block &B){sum_expressions.};
    // void restrict_sum_expressions(int i);
    // void compute_approximate_sum_expressions();
    // void compute_dense_sum_expressions();
