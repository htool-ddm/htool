...
#    pragma omp critical
    ...
    if (this->OffDiagonalApproximation != nullptr) {
        ...
        if (mu > 1 && !this->OffDiagonalApproximation->IsUsingRowMajorStorage()) { // Need to transpose input and output for OffDiagonalApproximation
            ...
            if (symmetry == 'H') {
                conj_if_complex(off_diagonal_input_column_major.data(), off_diagonal_input_column_major.size());
            }
            this->OffDiagonalApproximation->mvprod_off_diagonal_to_local(off_diagonal_input_column_major.data(), off_diagonal_out.data(), mu);
            ...
            this->OffDiagonalApproximation->mvprod_off_diagonal_to_local(off_diagonal_input.data(), off_diagonal_out.data(), mu);

            Blas<T>::axpy(&local_size_rhs, &da, off_diagonal_out.data(), &incx, out, &incy);
        }
    } else if (this->OffDiagonalApproximation != nullptr && this->OffDiagonalApproximation->IsUsingLocalVector()) {

        int local_source_size   = cluster_tree_s->get_local_size();
        int local_source_offset = cluster_tree_s->get_local_offset();

        // Build input vector (row major)
        std::vector<T> off_diagonal_input(local_source_size * mu, 0);
        std::copy_n(in + local_source_offset * mu, local_source_size * mu, off_diagonal_input.begin());
        std::vector<T> off_diagonal_out(cluster_tree_t->get_local_size() * mu, 0);

        if (mu > 1 && !this->OffDiagonalApproximation->IsUsingRowMajorStorage()) { // Need to transpose input and output for OffDiagonalApproximation

            std::vector<T> off_diagonal_input_column_major(off_diagonal_input.size(), 0);

            for (int i = 0; i < mu; i++) {
                for (int j = 0; j < local_source_size; j++) {
                    off_diagonal_input_column_major[j + i * local_source_size] = off_diagonal_input[i + j * mu];
                }
            }
            if (symmetry == 'H') {
                conj_if_complex(off_diagonal_input_column_major.data(), off_diagonal_input_column_major.size());
